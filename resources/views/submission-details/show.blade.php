...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="col-lg-6"> -->
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-8 col-lg-7">
                                ...
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3">
                                        <h6 class="m-0">Research Reviews</h6>
                                    </div>
                                    <div class="card-body">
                            ...
                            <div class="col-xl-4 col-lg-5">
                                <div class="card shadow mb-4">
                                    ...
                                    <div class="card-body">
                                        <form method="POST"
                                            @if (auth()->user()->role == 2) action="{{ route('store.summary.reviews', $records->id) }}">
                                                @csrf
                                                <input type="hidden" name="project_id" value="{{ $records->id }}">
                                                @elseif(auth()->user()->role == 4)
                                            ...
                                            <label for="contribution_to_knowledge">1. Does the paper contribute to the body
                                                of knowledge?</label>
                                    ...
                                    <div class="form-group">
                                        <label for="comprehensive_subject_matter">3. Is the subject matter presented in a
                                    ...
                                    <div class="form-group">
                                        <label for="applicable_and_sufficient_references">4. Are the references provided
                                    ...
                                    <div class="form-group">
                                        <label for="inappropriate_references">5. Are there references that are not
                                    ...
                                    <div class="form-group">
                                        <label for="grammar_and_presentation">7. Is the grammar and presentation poor?
                                    ...
                                    <div class="form-group">
                                        <label for="assumption_of_reader_knowledge">8. If the submission is very technical,
                                    ...
                                    <div class="form-group">
                                        <label for="clear_figures_and_tables">9. Are figures and tables clear and easy to
                                            interpret?</label>
                                        @if ($reviewerCommented > 0)
                                            @foreach ($revs as $review)
                                                @if (
                                    ...
                                    <div class="form-group">
                                        <label for="technical_or_methodological_errors">11. Are there any technical or
                                            methodological errors?</label>
                                        @if ($reviewerCommented > 0)
                                            @foreach ($revs as $review)
                                                @if (
                                                    $review->user->id === Auth::user()->id &&
                                                ...
                                                @if ($review->user->id === Auth::user()->id && $review->review_decision !== null && $review->project_id === $records->id)
                                                    <select class="form-control" id="review_decision"
                                                        ...
                                                        <option value="Accepted"
                                                            @if ($review->review_decision === 'Accepted') selected @endif>Accepted
                                                        </option>
                                                        <option value="Accepted with Revision"
                                                            @if ($review->review_decision === 'Accepted with Revision') selected @endif>Accepted with
                                                            Revision</option>
                                                        <option value="Rejected"
                                                            @if ($review->review_decision === 'Rejected') selected @endif>Rejected
                                                        </option>
                                                    </select>
                                                @endif
                                                @if ($review->user->id === Auth::user()->id && $review->review_decision === null && $review->project_id === $records->id)
            ...
            @else
                <div class="card shadow mb-4">
                    <div href="#collapseCardExample"
                        class="card-header py-3 d-flex justify-content-center align-items-center" data-toggle="collapse"
                    ...
                    <div class="card card-navy color-palette card-tabs">
                        <div class="card-header p-0">
                            <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" id="messages-btn" data-toggle="tab" href="#messages"
                          ...
                          <tr>
                              ...
                              <td>
                                  <a href="{{ route('submission-details.tasks.edit', $task->id) }}" type="button"
                                      ...
                                      data-keyboard="false" data-target="#editModal{{ $task->id }}">
                                      ...
                                      <div class="modal-dialog" role="document">
                                          <div class="modal-content">
                                              <div class="modal-header">
                                                  <h5 class="modal-title" id="editModal{{ $task->id }}Label">Edit
                                                      Task
                                                  </h5>
                ...
                <tbody>
                    @foreach ($lineItems as $lineItem)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('submission-details.line-items-budget.edit', $lineItem->id) }}"
                                    type="button" class="btn btn-warning" data-toggle="modal" data-backdrop="static"
                                    data-keyboard="false" data-target="#editModal{{ $lineItem->id }}">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <div class="modal fade" id="editModal{{ $lineItem->id }}" tabindex="-1" role="dialog"
                                    aria-labelledby="editModal{{ $lineItem->id }}Label" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModal{{ $lineItem->id }}Label">Edit
                                                    Line-Item Budget</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control"
                                                            id="edit_name{{ $lineItem->id }}" name="name"
                                                            value="{{ $lineItem->name }}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="edit_quantity">Quantity:</label>
                                                        <input type="number" class="form-control"
                                                            id="edit_quantity{{ $lineItem->id }}" name="quantity"
                                                            value="{{ $lineItem->quantity }}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="edit_unit_price">Unit Price:</label>
                                                        <input type="number" class="form-control"
                                                            id="edit_unit_price{{ $lineItem->id }}" name="unit_price"
                                                            value="{{ $lineItem->unit_price }}" required>
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
          ...
          <div class="card-footer">
            <div class="form-group float-center">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control bg-light text-large" id="total_all_line_items"
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{ route('submission-details.files.edit', $file->id) }}" type="button"
                                    class="btn btn-warning" data-toggle="modal" data-backdrop="static"
                                    data-keyboard="false" data-target="#editFileModal{{ $file->id }}">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <div class="modal fade" id="editFileModal{{ $file->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="editFileModal{{ $file->id }}Label"
                                    aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editFileModal{{ $file->id }}Label">Edit
                                                    File</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                            ...
                                            <form method="post"
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control"
                                                            id="edit_file_name{{ $file->id }}" name="file_name"
                ...
                @foreach ($teamMembers as $member)
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $member->role }}</td>
                            <td>
                                <a href="{{ route('submission-details.project-teams.edit', $member->id) }}"
                                    type="button" class="btn btn-warning" data-toggle="modal" data-backdrop="static"
                                    data-keyboard="false" data-target="#editModal{{ $member->id }}">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <div class="modal fade" id="editModal{{ $member->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="editModal{{ $member->id }}Label"
                                    aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModal{{ $member->id }}Label">Edit
                                            ...
                                            <form method="post"
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control"
                                                            id="edit_member_name{{ $member->id }}" name="member_name"
                                                            value="{{ $member->member_name }}" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="edit_role">Role:</label>
                                                        <select class="form-control" id="edit_role{{ $member->id }}"
                                                            name="role" required>
                                                            <option disabled>Select Role</option>
                                                            <option{{ $member->role === 'Project Leader' ? ' selected' : '' }}>
                                                                Project Leader</option>
                                                                <option{{ $member->role === 'Database Designer' ? ' selected' : '' }}>
                                                                    Database Designer</option>
                                                                    <option{{ $member->role === 'Network Designer' ? ' selected' : '' }}>
                                                                        Network Designer</option>
                                                                        <option{{ $member->role === 'UI Designer' ? ' selected' : '' }}>
                                                                            UI Designer</option>
                                                                            <option{{ $member->role === 'Quality Assurance' ? ' selected' : '' }}>
                                                                                Quality Assurance</option>
                                                                                <option{{ $member->role === 'Document Writer' ? ' selected' : '' }}>
                                                                                    Document Writer</option>
